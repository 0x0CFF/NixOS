...

{
  ...
  services.restic.backups = {
    ...
    Mnt-Live-Photo = {
      ...
      repository = "rclone:123pan-WebDAV:/Mnt/生活资料/相册";
      # 初始化仓库（第一次运行时创建，创建位置：rclone:123pan-WebDAV:/Mnt/生活资料/相册）
      initialize = true;
      # 指定 rclone 配置文件路径
    ...
    Mnt-Live-Video = {
      ...
      # 仓库地址：格式为 rclone:配置名:仓库路径
      repository = "rclone:123pan-WebDAV:/Mnt/生活资料/视频";
      # 初始化仓库（第一次运行时创建，创建位置：rclone:123pan-WebDAV:/Mnt/生活资料/视频）
      initialize = true;
      # 指定 rclone 配置文件路径
      # 通过 .path 引用解密后的 .age 文件（实际路径位于 /run/agenix/rclone-config）
      rcloneConfigFile = config.age.secrets.rclone-config.path;
    ...
    Mnt-Live-Audio = {
      ...
      # 仓库地址：格式为 rclone:配置名:仓库路径
      repository = "rclone:123pan-WebDAV:/Mnt/生活资料/音频";
      # 初始化仓库（第一次运行时创建，创建位置：rclone:123pan-WebDAV:/Mnt/生活资料/音频）
      initialize = true;
      # 指定 rclone 配置文件路径
      # 通过 .path 引用解密后的 .age 文件（实际路径位于 /run/agenix/rclone-config）
      rcloneConfigFile = config.age.secrets.rclone-config.path;
    ...
    Mnt-Live-Certificate = {
      ...
      # 仓库地址：格式为 rclone:配置名:仓库路径
      repository = "rclone:123pan-WebDAV:/Mnt/生活资料/证明";
      # 初始化仓库（第一次运行时创建，创建位置：rclone:123pan-WebDAV:/Mnt/生活资料/证明）
      initialize = true;
      # 指定 rclone 配置文件路径
      # 通过 .path 引用解密后的 .age 文件（实际路径位于 /run/agenix/rclone-config）
      rcloneConfigFile = config.age.secrets.rclone-config.path;
